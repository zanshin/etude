...
<?php if ($comments) : ?>
	...
	<?php foreach ($comments as $comment) : ?>
		...
		<li class="<?php echo $oddcomment; /* Style differently if comment author is blog author */ if ($comment->comment_author_email == get_the_author_email()) { echo ' authorcomment'; } ?>" id="comment-<?php comment_ID() ?>">
			<div class="cmtinfo"><em><?php edit_comment_link('edit this','',''); ?> on <?php comment_date('d M Y') ?> at <?php comment_time() ?></em><small class="commentmetadata"><a href="#comment-<?php comment_ID() ?>" title=""><span class="commentnum"><?php echo $commentcounter; ?></span></a></small><cite><?php comment_author_link() ?></cite></div>
			<?php if ($comment->comment_approved == '0') : ?>
			<em>Your comment is awaiting moderation.</em><br />
			<?php endif; ?>		
	                <div style="float:left;"><?php echo get_avatar($comment, $size=64) ?></div>
			<?php comment_text() ?>	
                        <br /><br />  <!-- for one line comments -->		
		</li>
